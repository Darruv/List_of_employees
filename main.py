...
class Main(tk.Frame):
    ...
    def init_main(self):
        ...
        btn_open_dialog = tk.Button(toolbar, text='Добавить', bd=3, command=self.open_dialog)
        # упаковка и выравнивание по левому краю
        btn_open_dialog.pack(side=tk.LEFT)

        # создание кнопки изменения данных
        btn_edit_dialog = tk.Button(toolbar, text='Изменить', bd=3, command=self.open_update_dialog)
        btn_edit_dialog.pack(side=tk.LEFT)

        # создание кнопки удаления записи
        btn_delete = tk.Button(toolbar, text='Удалить', bd=3, command=self.delete_records)
        btn_delete.pack(side=tk.LEFT)

        # кнопка поиска
        btn_search = tk.Button(toolbar, text='Поиск', bd=3, command=self.open_search_dialog)
        btn_search.pack(side=tk.LEFT)

        # кнопка обновления
        btn_refresh = tk.Button(toolbar, text='Обновить', bd=3, command=self.view_records)
